...
def prepare_trees_with_cuts(filenameOut = "Output.root", list_of_trees = [""], is_data = False,channel = 'ele'):
...
for ch in chan:
  ...
  prepare_trees_with_cuts("treeEDBR_data_xww.root", data_list_of_trees, True, ch)
  ...
  prepare_trees_with_cuts("treeEDBR_WJets_xww.root", W_list_of_trees, False, ch)

  T_list_of_trees = ["s-ch_%s.root"%(ch), "tW-ch-antitop_%s.root"%(ch), "tW-ch-top_%s.root"%(ch), "t-ch_%s.root"%(ch)]
  prepare_trees_with_cuts("treeEDBR_SingleTop_xww.root", T_list_of_trees, False, ch)

  VV_list_of_trees = ["WW_%s.root"%(ch), "WZ_%s.root"%(ch)]
  prepare_trees_with_cuts("treeEDBR_VV_xww.root", VV_list_of_trees, False, ch)

  TTBAR_list_of_trees = ["ttbar_%s.root"%(ch)]
  prepare_trees_with_cuts("treeEDBR_TTBARpowheg_xww.root", TTBAR_list_of_trees, False, ch)
