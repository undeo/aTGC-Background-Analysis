...
fileInWS	= TFile.Open('TMPTMP/cards_%s_HP%s/mj_workspace.root'%(ch,cat[1]))
w		= fileInWS.Get('workspace4fit_')
fileInWS.Close()
...
if options.nuis:
	...
	fileOut		= TFile.Open("mj_closure/datasetwjet_%s_HP%s.root"%(ch,cat[1]),"recreate")
else:
...
if options.onlywjets:
	...
	WJets_norm_tmp  = int(WJets_norm_val * 2)
	print VV_norm_tmp
	VV_pdf.Print()
	rrv_x.Print()
	...
	allbkg_dataset = RooDataSet('allbkg_dataset_2wjets','allbkg_dataset_2wjets',RooArgSet(rrv_x))
	...
	WJets_norm_tmp  = int(WJets_norm.getVal() * 0.5)
	...
	allbkg_dataset = RooDataSet('allbkg_dataset_05wjets','allbkg_dataset_05wjets',RooArgSet(rrv_x))
	allbkg_dataset.append(VV_dataset)
	allbkg_dataset.append(STop_dataset)
	allbkg_dataset.append(TTbar_dataset)
	allbkg_dataset.append(WJets_dataset)
	...
	for i in range(100):
		...
		if options.nuis:
			VV_pars 	= w.genobj('fitresult_model_VV_xww_%s_mj_rdataset4fit_VV_xww_%s_mj'%(ch,ch)).randomizePars()
			for j in range(VV_pars.getSize()):
				if 'number' not in VV_pars[j].GetName():
					w.var(VV_pars[j].GetName()).setVal(VV_pars[j].getVal())
			STop_pars	= w.genobj('fitresult_model_STop_xww_%s_mj_rdataset4fit_STop_xww_%s_mj'%(ch,ch)).randomizePars()
			for j in range(STop_pars.getSize()):
				if 'number' not in STop_pars[j].GetName():
					w.var(STop_pars[j].GetName()).setVal(STop_pars[j].getVal())
			TTbar_pars	= w.genobj('fitresult_model_TTbar_xww_%s_mj_rdataset4fit_TTbar_xww_%s_mj'%(ch,ch)).randomizePars()
			...
			WJets_pars	= w.genobj('fitresult_model_WJets0_xww_%s_mj_rdataset4fit_WJets0_xww_%s_mj'%(ch,ch)).randomizePars()
